...
def _rnn_name_replacement(var_name):
  for pattern in RNN_NAME_REPLACEMENTS:
    if pattern in var_name:
      ...
      print('Converted: %s --> %s' % (old_var_name, var_name))
      break
  return var_name
...
def convert_names(checkpoint_from_path,
  ...
  with ops.Graph().as_default():
    print('Reading checkpoint_from_path %s' % checkpoint_from_path)
    ...
    with session.Session() as sess:
      ...
      print('Writing checkpoint_to_path %s' % checkpoint_to_path)
      saver.save(sess, checkpoint_to_path)

  print('Summary:')
  print('  Converted %d variable name(s).' % len(new_variable_map))
